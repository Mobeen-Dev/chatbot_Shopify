...
class Shopify:
    ...
    async def send_storefront_mutation(
        self, mutation: str, variables: dict, receiver: str = "child"
    ):
        ...
        try:
            ...
            if "errors" in result:
                ...
                return await self.send_storefront_mutation(
                    mutation, variables, receiver
                )
                ...
                if root_errors:
                    ...
                    if code == "HANDLE_NOT_UNIQUE":
                        ...
                        if match:
                            ...
                            return await self.send_graphql_mutation(
                                mutation, variables, receiver
                            )
    ...
    async def create_cart(
        self, items: list[dict[str, str | int]], session_id: str = "default"
    ):  # [ {"handle": "product-alpha", "variant":"Default Title", "qty" : 123 } ]
  ...
  """
        ...
        for obj in items:
            ...
            if merchandise_id:
                lines.append({"quantity": qty, "merchandiseId": merchandise_id})
            else:
                variant_error = True
                errors.append(
                    {
        ...
        variables = {
            ...
            #   }
            # },
            "attributes": [{"key": "Chat #", "value": f"{session_id}"}],
    ...
    async def addCartLineItems(
        self, cartId: str, lineItems: List[dict[str, str | int]]
    ):
        mutation = """
  ...
  """
        ...
        variables = {"cartId": cartId, "lineIds": lines}
        result = await self.send_storefront_mutation(mutation, variables)
  ...
  """
    ...
    async def fetch_mapping_products(self):
        ...
        while hasNextPage:
            try:
                result = await self.send_graphql_mutation(
                    query, query_params, "GetProductsAndVariants"
                )
    ...
    """
        ...
        if errs := response.get("errors"):
            self.logger.error(
                f"fetch_product_by_id::GraphQL errors: {errs}"
            )  # optional
            return None
    ...
    async def fetch_order_by_name(self, order_name: str):
        ...
        variables = {"startCursor": after_cursor, "query": query_string}
    ...
    async def product_id_by_handle(self, product_handle: str):
        ...
        query_params = {"identifier": {"handle": f"{product_handle}"}}
    ...
    async def send_graphql_mutation(
        self, mutation: str, variables: dict, receiver: str = "child"
    ):
        try:
            async with aiohttp.ClientSession() as session:
    ...
    async def sync_product(self, p_id: int = 404, child_p_id: int = 404):
        ...
        # print(product)
        query_params = self.parse_into_query_params(
            product, f"gid://shopify/Product/{child_p_id}"
        )
    ...
    async def set_product_status(self, id: int = 404, status: str = "DRAFT"):
        ...
        query_params = {"id": f"gid://shopify/Product/{id}", "status": status}
        # query_params = parse_response_to_query(product, "gid://shopify/Product/8872805433568") #Update
    ...
    async def make_new_customer(self, customer: dict):
        ...
        try:
            ...
            self.logger.warning(
                f"fetch_product_by_id :: Failed to create new customer ::{e}"
            )
            return None
    ...
    def format_product(self, product: dict, Strict=True) -> dict:
        ...
        all_variant_outOfStock = self.all_variant_outOfStock(variants)
        ...
        if status == "ACTIVE" or not Strict:
            ...
            return {
                ...
                "priceRange": {
                    "CurrencyCode": product.get("priceRangeV2", {})
                    .get("maxVariantPrice", {})
                    .get("currencyCode", ""),
                    "max_price": product.get("priceRangeV2", {})
                    .get("maxVariantPrice", {})
                    .get("amount", 0),  # removing .00 so that only actual price came
                    "min_price": product.get("priceRangeV2", {})
                    .get("minVariantPrice", {})
                    .get("amount", 0),
                },
        ...
        def handle_variants(vary_items):
            ...
            for variant in variants:
                parent_variant = [
                    ...
                    if variant in option.get("values", [])
                ]
                    ...
                    "files": [
                        {
                            ...
                            "originalSource": image["node"]["image"]["url"].split(
                                "?", 1
                            )[0],
                        }
                        for image in images
                    ...
                    "variants": [
                        {
                            ...
                            "inventoryItem": {
                                ...
                                # "cost": 12.34,
                                "requiresShipping": variant["node"]["inventoryItem"][
                                    "requiresShipping"
                                ],
                                "measurement": variant["node"]["inventoryItem"][
                                    "measurement"
                                ],
                            },
                        }
